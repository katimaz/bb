...
class WebController extends Controller
{
	...
	public function login_pt(Request $request)
    {
		if ($request->has('email') && $request->email && $request->has ( 'password' ))
		  ...
		  {
			  $user = User::where('email', '=', trim($request->email))->select('id','usr_id','password','usr_status','last_name','first_name','phone_number','email','remember_token','cookie_id','open_offer_setting')->first();
			  $password = trim($request->password) . ":" . $user->usr_id;
	...
	public function profile_pt(Request $request)
    {
		...
		$new_user = User::where('usr_id',$account)->select('id','usr_id','usr_status','email','last_name','first_name','open_offer_setting')->first();
		if(!$new_user)
			return View('web/error', array('message' => '很抱歉，無此帳號權限喔! 請重新登入'));
...
>>>>>>> a70e6bd64252ad276ccb86afa867850e14e8f158
}